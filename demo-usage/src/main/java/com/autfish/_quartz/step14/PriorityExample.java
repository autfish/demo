package com.autfish._quartz.step14;
...
public class PriorityExample {
	...
	public void run() throws Exception {
		...
		JobDetail job = newJob(TriggerEchoJob.class).withIdentity(
				"TriggerEchoJob").build();
		...
		Trigger trigger1 = newTrigger()
				...
				.withSchedule(
						simpleSchedule().withRepeatCount(1)
								.withIntervalInSeconds(5)).withPriority(1)
				.forJob(job).build();

		// Second trigger has default priority of 5 (default), and will repeat
		// after 10 seconds
		Trigger trigger2 = newTrigger()
				...
				.withSchedule(
						simpleSchedule().withRepeatCount(1)
								.withIntervalInSeconds(10)).forJob(job).build();
		...
		Trigger trigger3 = newTrigger()
				...
				.withSchedule(
						simpleSchedule().withRepeatCount(1)
								.withIntervalInSeconds(15)).withPriority(10)
				.forJob(job).build();
