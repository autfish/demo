package com.autfish._quartz.step15;
 ...
 * are synchronized using some form of time-sync service (daemon).
 * </p>
 * 
 * @author James House
 */
	...
	public void run(boolean inClearJobs, boolean inScheduleJobs)
		...
		if (inScheduleJobs) {
			...
			JobDetail job = newJob(SimpleRecoveryJob.class)
					.withIdentity("job_" + count, schedId) // put triggers in
															// group
															// named after the
															// cluster
															// node instance
			...
			SimpleTrigger trigger = newTrigger()
					...
					.withSchedule(
							simpleSchedule().withRepeatCount(20)
									.withIntervalInSeconds(5)).build();

			_log.info(job.getKey() + " will run at: "
					+ trigger.getNextFireTime() + " and repeat: "
