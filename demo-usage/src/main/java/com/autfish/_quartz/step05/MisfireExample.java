package com.autfish._quartz.step05;
 ...
 * misfire instructions affect the firings of triggers of
 * <code>StatefulJob</code> s - when the jobs take longer to execute that the
 * frequency of the trigger's repitition.
 * <p>
 ...
 * seconds). You should see that one of the jobs has its misfire instruction set
 * to
 * <code>SimpleTrigger.MISFIRE_INSTRUCTION_RESCHEDULE_NOW_WITH_EXISTING_REPEAT_COUNT</code>
 * , which causes it to fire immediately, when the misfire is detected. The
 * other trigger uses the default "smart policy" misfire instruction, which
 * causes the trigger to advance to its next fire time (skipping those that it
 * has missed) - so that it does not refire immediately, but rather at the next
 * scheduled time.
 * </p>
 * 
...
public class MisfireExample {
	...
	public void run() throws Exception {
		...
		// (but it will delay for ten seconds - and therefore purposely misfire
